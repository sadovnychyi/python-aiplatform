    ...
    def find_neighbors(
        ...
        *,
        ...
        Example usage:
            ...
            my_index_endpoint.find_neighbors(deployed_index_id="index_id", queries= [[1, 1]],)
        ```
        Args:
            deployed_index_id (str):
    ...
    def match(
        ...
        Args:
            ...
            embedding_id (str):
               ...
               `embedding_id` to lookup embedding values from dataset, if embedding
               with `embedding_id` exists in the dataset, do nearest neighbor search.
        ...
        if queries:
            ...
            if filter:
                for namespace in filter:
                    ...
                    restrict.deny_tokens.extend(namespace.deny_tokens)
                    request.restricts.append(restrict)
            batch_request_for_index.requests.append(request)
        else:
            raise ValueError(
                "To find neighbors using matching engine,"
                "please specify `queries` or `embedding_id`"
            )
